...
draw = function(divID, array){
  ...
  for(i=0;i <= (d-1)*d; i += d){
    for(j=i; j <= (i+d-1); j++){
      document.getElementById(divID).innerHTML += '<div class="cell" id="' + j + '">' + array[j] + '</div>';
    }
  ...
  if(result.end == 'win'){
    ...
    if(result.winType == 'row'){
      for(i=result.point; i <= result.point + d; i++){
        toggleWin(i);
      }
    }
  }else{
...
function parseTextInside(elmnt){
  array = document.getElementByID(elmnt).innerHTML;
